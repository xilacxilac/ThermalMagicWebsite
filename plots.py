...
def signal_generation(g00, f0, f1, dia, Ms, Bs, lam, noise):
    global Tnew
    lam = 10**lam
    ...
    #*********
    #
    #   Added by MAH 8th of Jan
    #
    numpart = 3.168*10**20*dx*dy*dz # we need this number later to scale up the reconstruction
    #
    #*********
    for i in range(nx):
        for j in range(ny):
            for k in range(nz):
                ...
                if dist<0.003 and dist>0.001:
                    Rho[i,j,k,0] = numpart #** changed by MAH
    ...
    plt.title('Particle Magnetization vs Field Response')
    plt.plot(Hs, x)
    plt.xlabel('H')
    plt.ylabel('Ms')
    ...
    plt.imshow(usort2D.T,extent=extent,cmap='jet')
    plt.xlabel('x [m]')
    plt.ylabel('y [m]')
    ...
    #**************************
    #
    #   Added by MAH, 8th of January. This is a more accurate, yet slower, reconstruction method
    #
    RhoCal = np.zeros((nx,ny,1,4))
    for i in range(nx):
        for j in range(ny):
            for k in range(nz):
                RhoCal[i,j,k,1] = 295
        ...
        for j in range(ny):
            ...
            for i1 in range(nx):
                for j1 in range(ny):
                    ...
                    T[kindx1,kindx2] = MvHfunDer(Htmp/Hsat)/Hsat
                    kindx2 = kindx2 + 1
            kindx1 = kindx1 + 1
    ...
    #*****************
    #
    #   Changed by MAH, 8th of January, perform the more accurate reconstruction
    #
    Sfull = np.zeros((nt*2)); Sfull[0:nt] = S[:,0]; Sfull[nt:2*nt] = S[:,1]
    print('Shape of S: {0}'.format(Sfull.shape)) 
    fit = model.fit(Tnew,Sfull)
    Rho_hat = numpart*fit.coef_
...
def lambda_dependent_graphs(g00, f0, f1, dia, Ms, Bs, lam, noise):
    lam = 10**lam
